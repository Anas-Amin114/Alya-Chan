...
// Safely load or create fishing.json
if (!fs.existsSync(fishingDataFile)) {
  console.log("fishing.json does not exist. Creating a new file...");
  fishingData = {};
  fs.writeFileSync(fishingDataFile, JSON.stringify(fishingData, null, 2), 'utf8');
} else {
  try {
    const data = fs.readFileSync(fishingDataFile, 'utf8');
    fishingData = JSON.parse(data);
  } catch (error) {
    console.error("Error reading or parsing fishing.json. Resetting to empty data:", error);
    fishingData = {};
    fs.writeFileSync(fishingDataFile, JSON.stringify(fishingData, null, 2), 'utf8');
  }
}
  ...
  onStart: async function ({ api, args, message, event, usersData }) {
    try {
      ...
      if (args.length === 0) {
        ...
        fishingData[senderID].fishingCount += 1;
